		...
		for(Entry<String, String> entry : query.entrySet()) {
		    ...
		    for(Entry<String, String> tt : time.entrySet()) {
		         ...
		         String times = tt.getValue();
			         if (key==title2) {
			        	 tree.root.addChild(new WebNode(new WebPage(value,key,times)));
			         }
			}
		    
		    ChildQuery childweb = new ChildQuery(value);
		   ...
		   try {
			   ...
			   if(!childweb.childurl1.equals("noChild")) {
				   ...
				   tree.root.children.get(childNum).addChild(new WebNode(new WebPage(childweb.childurl1,"kid1","times")));
//					System.out.println(childweb.childurl1);
				   tree.root.children.get(childNum).addChild(new WebNode(new WebPage(childweb.childurl2,"kid2","times")));
//					System.out.println(childweb.childurl2);
		...
		if(request.getParameter("keyword").contains(" ")) {
			...
			for(String keyword: keywords) {
				keywordList.add(new Keyword(keyword,10));
